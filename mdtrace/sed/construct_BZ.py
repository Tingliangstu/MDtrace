...
class BZ_methods(object):
    ...
    def _compare_cell(self, params, box_in_traj):
        ...
        if np.allclose(box_in_traj, expected_box, atol=1e-4):
            ...
            if self.box_info:
                print(
                    "\n⚠️ mdtrace identified that structural optimization is likely performed using the NVT ensemble.")
            ...
            if params.rescale_prim:
                ...
                if self.box_info:
                    print("\nNow, mdtrace have reconstructed primitive unit cell matrix from MD simulation cell (Angstrom):")
                    for row in params.prim_unitcell:
                        print("  {: .8e}    {: .8e}    {: .8e}".format(row[0], row[1], row[2]))
