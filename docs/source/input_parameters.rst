...
mdtrace reads a plain-text control file. The default file name is
...
command line, mdtrace reads ``input_SED.in`` in the current directory:
...
The table below lists the parameters accepted by the mdtrace input file. Each
...
.. list-table::
   ...
   * - `out_files_name <input_parameters/out_files_name.html>`_
     - Files
     - Prefix for mdtrace output files.
